...
namespace UnitTest
{
    public class UnitTest1
    {
        ...
        public void ClienteRealizaAplicacaoFinanceiraComSucesso1()
        {
            ...
            Assert.Equal(4.5m, informacaoFiscal.ValorRetido);
            Assert.Equal(515.5m, informacaoFiscal.Produto.Saldo);
        ...
        public void ClienteRealizaAplicacaoFinanceiraNaFaixa225ComSucesso()
        {
            ...
            var produto1 = new Produto { Data = new DateTime(2019, 1, 1), Nome = "CDB", PercentualRedimento = 2, Saldo = 1000 };
            var produto2 = new Produto { Data = new DateTime(2019, 1, 1), Nome = "RENDAFIXA", PercentualRedimento = 2, Saldo = 1000 };
            var produtos = new List<Produto>();
            produtos.Add(produto1);//Vários produtos podem ser comercializados
            produtos.Add(produto2);
            var cliente = new Cliente { Nome = "Valnei Filho" };
            var servico = new Servico(cliente, TipoCliente.Especial);
            servico.ClienteRealizaAplicacao(produtos);//APlicar dinheiro em produtos
            var informacaoFiscal = servico.ClienteRealizaResgate(500,1, DateTime.Now.Date);//Resgatar CDB
        ...
        public void ClienteRealizaAplicacaoFinanceiraNaFaixa2ComSucesso()
        {
            ...
            Assert.Equal(7.4m, informacaoFiscal.ValorRetido);
            Assert.Equal(532.6m, informacaoFiscal.Produto.Saldo);
        ...
        public void ClienteRealizaAplicacaoFinanceiraNaFaixa3ComSucesso()
        {
            ...
            //4 anos de aplicação
            Assert.Equal(80, informacaoFiscal.RedimentoAplicacao);
            Assert.Equal(12, informacaoFiscal.ValorRetido);
            Assert.Equal( 568m, informacaoFiscal.Produto.Saldo);
